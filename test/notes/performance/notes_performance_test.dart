  ...
  group('Notes Performance Tests', () {
    ...
    group('Note Creation Performance', () {
      ...
      test('should create multiple notes efficiently', () async {
        ...
        expect(stopwatch.elapsedMilliseconds,
            lessThan(noteCount * 50)); // Average 50ms per note

        final avgTimePerNote = stopwatch.elapsedMilliseconds / noteCount;
        expect(
            avgTimePerNote, lessThan(AnchoringConstants.maxReanchoringTimeMs));
      });
    });
    ...
    group('Note Loading Performance', () {
      test('should load book notes within performance target', () async {
        ...
        const bookText =
            'Performance test content for book loading with multiple notes.';
        ...
        expect(stopwatch.elapsedMilliseconds,
            lessThan(500)); // Should load 20 notes in under 500ms
      ...
      test('should load from cache very quickly', () async {
        ...
        expect(stopwatch.elapsedMilliseconds,
            lessThan(10)); // Cache should be very fast
      });
      ...
      test('should handle visible range efficiently', () async {
        ...
        const visibleRange =
            VisibleCharRange(2000, 3000); // Should include ~10 notes
        ...
        expect(
            stopwatch.elapsedMilliseconds, lessThan(5)); // Should be very fast
      });
    });
    ...
    group('Search Performance', () {
      test('should search notes within performance target', () async {
        ...
        expect(stopwatch.elapsedMilliseconds,
            lessThan(200)); // Should search in under 200ms
      });
      ...
      test('should handle complex search queries efficiently', () async {
        ...
        // Test complex search
        final stopwatch = Stopwatch()..start();
        final results = await integrationService.searchNotes('complex test',
            bookId: bookId);
        stopwatch.stop();

        expect(results.length, greaterThan(0));
        expect(stopwatch.elapsedMilliseconds,
            lessThan(300)); // Complex search under 300ms
      });
    });
    ...
    group('Batch Processing Performance', () {
      test('should process batch operations efficiently', () async {
        ...
        expect(stopwatch.elapsedMilliseconds,
            lessThan(1000)); // 10 updates in under 1 second

        final avgTimePerUpdate = stopwatch.elapsedMilliseconds / 10;
        expect(
            avgTimePerUpdate, lessThan(100)); // Average under 100ms per update
      });
    ...
    group('Stress Tests', () {
      ...
      test('should handle concurrent operations', () async {
        ...
        expect(stopwatch.elapsedMilliseconds,
            lessThan(20 * 100)); // Much faster than sequential
      });
      ...
      test('should maintain performance with large visible ranges', () async {
        ...
        for (final range in ranges) {
          final stopwatch = Stopwatch()..start();
          final visibleNotes =
              integrationService.getNotesForVisibleRange(bookId, range);
          stopwatch.stop();

          expect(visibleNotes.length, greaterThan(0));
          expect(stopwatch.elapsedMilliseconds,
              lessThan(50)); // Should be fast regardless of range size
        }
      });
    });
    ...
    group('Performance Regression Tests', () {
      test('should maintain consistent anchoring performance', () async {
        ...
        final avgTime =
            anchoringTimes.reduce((a, b) => a + b) / anchoringTimes.length;
        ...
        final variance = anchoringTimes
                .map((time) => (time - avgTime) * (time - avgTime))
                .reduce((a, b) => a + b) /
            anchoringTimes.length;
        expect(variance,
            lessThan(1000)); // Low variance indicates consistent performance
      });
      ...
      test('should maintain search performance with growing dataset', () async {
        ...
        for (final count in noteCounts) {
          // Add more notes
          final currentNoteCount = integrationService
              .getNotesForVisibleRange(
                  bookId, const VisibleCharRange(0, 999999))
              .length;
          final notesToAdd = count - currentNoteCount;

          for (int i = currentNoteCount;
              i < currentNoteCount + notesToAdd;
              i++) {
            await integrationService.createNoteFromSelection(
              bookId,
              'search regression $i',
        ...
        expect(
            searchTimes.last,
            lessThan(searchTimes.first *
                4)); // Not more than 4x slower with 20x data
        expect(searchTimes.every((time) => time < 500),
            isTrue); // All searches under 500ms
      });
    });
  });
}
