  ...
  group('Notes Acceptance Tests', () {
    ...
    group('User Story: Viewing and Managing Notes', () {
      test('As a user, I can view all my notes for a book', () async {
        ...
        expect(bookNotes.notes.map((n) => n.id),
            containsAll([note1.id, note2.id, note3.id]));

        // Notes should be sorted by position
        expect(bookNotes.notes[0].charStart,
            lessThan(bookNotes.notes[1].charStart));
        expect(bookNotes.notes[1].charStart,
            lessThan(bookNotes.notes[2].charStart));
      });

      test('As a user, I can see notes only in the visible text range',
          () async {
    ...
    group('User Story: Editing and Updating Notes', () {
      test('As a user, I can edit the content of my notes', () async {
        ...
        expect(updatedNote.contentMarkdown,
            equals('Updated content with more details'));
        expect(updatedNote.charStart,
            equals(originalNote.charStart)); // Position unchanged
    ...
    group('User Story: Searching Notes', () {
      test('As a user, I can search for notes by content', () async {
        ...
        final fruitResults =
            await integrationService.searchNotes('fruit', bookId: bookId);
        final treeResults =
            await integrationService.searchNotes('tree', bookId: bookId);
      ...
      test('As a user, I can search for notes in Hebrew', () async {
        ...
        // Search in Hebrew
        final appleResults =
            await integrationService.searchNotes('תפוח', bookId: bookId);
        final fruitResults =
            await integrationService.searchNotes('פירות', bookId: bookId);
      ...
      test('As a user, I can search across multiple books', () async {
        ...
        final commonResults =
            await integrationService.searchNotes('common term');
        final uniqueResults =
            await integrationService.searchNotes('unique term');

        expect(commonResults.length, equals(2));
        expect(commonResults.map((n) => n.bookId),
            containsAll([book1Id, book2Id]));
    ...
    group('User Story: Deleting Notes', () {
      test('As a user, I can delete notes I no longer need', () async {
        ...
        final searchResults =
            await integrationService.searchNotes('deleted', bookId: bookId);
      ...
      test('As a user, deleting a note removes it from all views', () async {
        ...
        expect(
            visibleNotes.map((n) => n.id), containsAll([note1.id, note3.id]));
        ...
        expect(
            highlights.map((h) => h.noteId), containsAll([note1.id, note3.id]));
    ...
    group('User Story: Import and Export Notes', () {
      test('As a user, I can export my notes to backup them', () async {
        ...
        expect(
            exportResult.jsonData!, contains('"tags": ["export", "backup"]'));
      ...
      test('As a user, I can import notes from a backup', () async {
        ...
        final exportResult =
            await importExportService.exportNotes(bookId: originalBookId);
        expect(exportResult.success, isTrue);
        ...
        final targetNotes = await integrationService.loadNotesForBook(
            targetBookId, targetBookText);
        expect(targetNotes.notes.length, equals(1));
        expect(targetNotes.notes.first.bookId, equals(targetBookId));
        expect(targetNotes.notes.first.contentMarkdown,
            equals('Note for import test'));
    ...
    group('User Story: Handling Text Changes and Re-anchoring', () {
      test('As a user, my notes stay accurate when text has minor changes',
          () async {
        ...
        final modifiedBookNotes =
            await integrationService.loadNotesForBook(bookId, modifiedText);
        ...
        expect([NoteStatus.anchored, NoteStatus.shifted],
            contains(reanchoredNote.status));
      });
      ...
      test('As a user, I am notified when notes become orphaned', () async {
        ...
        // Create note
        final note = await integrationService.createNoteFromSelection(bookId,
            'will be orphaned', 10, 26, 'This note will become orphaned');
    ...
    group('Accuracy Requirements Validation', () {
      test('should achieve 98% accuracy after 5% text changes', () async {
        ...
        const modifiedText =
            'This is a comprehensive test document with multiple sentences. Each sentence contains different words and phrases. The document is used for testing note anchoring accuracy.';
        ...
        // Calculate accuracy
        final anchoredCount = modifiedBookNotes.notes
            .where((n) =>
                n.status == NoteStatus.anchored ||
                n.status == NoteStatus.shifted)
            .length;
        final accuracy = anchoredCount / notes.length;

        expect(
            accuracy, greaterThanOrEqualTo(0.98)); // 98% accuracy requirement
      });
      ...
      test('should achieve 100% accuracy for whitespace-only changes',
        ...
        final modifiedBookNotes = await integrationService.loadNotesForBook(
            bookId, whitespaceModifiedText);

        expect(modifiedBookNotes.notes.length, equals(1));
        expect(modifiedBookNotes.notes.first.status,
            equals(NoteStatus.anchored)); // Should remain anchored
      });
      ...
      test('should handle deleted text properly', () async {
        ...
        final modifiedBookNotes =
            await integrationService.loadNotesForBook(bookId, deletedText);

        expect(modifiedBookNotes.notes.length, equals(1));
        expect(modifiedBookNotes.notes.first.status,
            equals(NoteStatus.orphan)); // Should be orphaned
      });
    });
    ...
    group('Text Normalization Consistency', () {
      test('should produce consistent normalization results', () {
        ...
        final normalizedResults = testTexts
            .map((text) => TextNormalizer.normalize(text, config))
            .toList();
      ...
      test('should handle Hebrew nikud consistently', () {
        ...
        final normalizedWithNikud =
            TextNormalizer.normalize(textWithNikud, configKeepNikud);
        final normalizedWithoutNikud =
            TextNormalizer.normalize(textWithNikud, configRemoveNikud);
        final originalWithoutNikud =
            TextNormalizer.normalize(textWithoutNikud, configRemoveNikud);

        expect(normalizedWithNikud, contains('ְ')); // Should contain nikud
        expect(normalizedWithoutNikud,
            isNot(contains('ְ'))); // Should not contain nikud
        expect(normalizedWithoutNikud,
            equals(originalWithoutNikud)); // Should match text without nikud
      });
    });
    ...
    group('Fuzzy Matching Validation', () {
      test('should meet similarity thresholds', () {
        ...
        final similarityHigh =
            FuzzyMatcher.calculateCombinedSimilarity(originalText, similarText);
        final similarityLow = FuzzyMatcher.calculateCombinedSimilarity(
            originalText, differentText);

        expect(
            similarityHigh, greaterThan(AnchoringConstants.jaccardThreshold));
      ...
      test('should handle Hebrew text similarity correctly', () {
        ...
        final similarityHigh = FuzzyMatcher.calculateCombinedSimilarity(
            hebrewOriginal, hebrewSimilar);
        final similarityLow = FuzzyMatcher.calculateCombinedSimilarity(
            hebrewOriginal, hebrewDifferent);
